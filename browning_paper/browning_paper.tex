...
\todo[inline]{I'm not actually sure this is correct--the Harris et al.~\cite{harris:2015} paper states "The first step in employing a hyper-heuristic is creating a set of algorithmic primitives appropriate for tackling a specific problem class. The second step is searching the associated algorithmic primitive space. Hyper-heuristics have typically employed Genetic Programming (GP) to execute the second step, but even in GP there are many alternatives." I'm not sure this is correct because wouldn't GP be generating the primitives AND searching that space? or are the primitives supposed to be parameters and GP searches through all possible combinations of those parameters in order to find a solution....?}
